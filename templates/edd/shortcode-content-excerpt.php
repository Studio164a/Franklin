...
if ($output === false ) :
	...
	<div class="campaign block entry-block cf">
		...
		if ( has_post_thumbnail( $campaign->ID ) ) : 
			?>			
			<div class="campaign-image"><?php 
				// Campaign ended, fully funded
				if ( $campaign->is_funded() ) : 
					?><span class="campaign-successful"><?php _e( 'Successful', 'franklin' ) ?></span><?php 
		...
		<ul class="campaign-status horizontal center">
			...
			<li class="campaign-raised">
				<span><?php echo $campaign->percent_completed(false) ?><sup>%</sup></span><?php _e( 'Funded', 'franklin' ) ?>		
			</li>
			<li class="campaign-pledged">
				<span><?php echo $campaign->current_amount() ?></span><?php _e( 'Pledged', 'franklin' ) ?>				
			</li>
